...
def init_driver():
    ...
    # For headless: opts.add_argument("--headless=new")
    driver = webdriver.Chrome(options=opts)
    cookie_file = 'linkedin_cookies.json'

    # Try cookie login
    success = import_linkedin_cookies(driver, cookie_file)
    if not success:
        log.warning("Cookies failed or expired. Please log in manually in the browser window...")
        driver.get("https://www.linkedin.com/login")
        input("After logging in successfully, press Enter here to continue...")  # Wait for user to login

        # Save cookies for future runs!
        save_cookies(driver, cookie_file)
        if not is_logged_in(driver):
            raise RuntimeError("❌ Manual login also failed. Please check credentials.")
        else:
...
def import_linkedin_cookies(driver, cookie_file='linkedin_cookies.json'):
    """Load LinkedIn cookies (EditThisCookie JSON export) into Selenium driver."""
    ...
    for cookie in cookies:
        ...
        # Remove 'session' if 'expiry' is set
        if 'expiry' in cookie:
    ...
    driver.refresh()  # Important! Ensure session is active.
    time.sleep(2)
    driver.get("https://www.linkedin.com/feed/")
    time.sleep(3)
    # Verify login
    if not is_logged_in(driver):
        print("⚠  Not logged in! Cookies may be expired or invalid.")
        return False
    else:
...
def is_logged_in(driver):
    """Check if logged in to LinkedIn feed."""
    url = driver.current_url
    # LinkedIn will redirect to /feed/ if logged in, /login otherwise
    return "feed" in url and "login" not in url and "checkpoint" not in url


def save_cookies(driver, cookie_file='linkedin_cookies.json'):
    """Save current cookies in EditThisCookie-compatible JSON format."""
    cookies = driver.get_cookies()
    with open(cookie_file, 'w') as f:
        json.dump(cookies, f, indent=2)
    log.info(f"✅ Saved fresh cookies to {cookie_file}")

# def is_logged_in(driver):
#     """
#     Returns True if currently logged into LinkedIn.
#     """
#     driver.get("https://www.linkedin.com/feed/")
#     time.sleep(2)
#     url = driver.current_url
#     # "feed" should be in url if already logged in, otherwise it'll redirect to login/checkpoint
#     return ("feed" in url and "login" not in url and "checkpoint" not in url)
...
#                 driver.add_cookie(cookie)
#             except Exception as e:
#                 log.debug(f"Error adding cookie: {e}")
#         driver.get("https://www.linkedin.com/feed/")
#         time.sleep(2)
#         if is_logged_in(driver):
#             log.info(f"✅ Already logged in via cookies for {account['username']}")
#             return
#         else:
#             log.warning(f"Cookies expired or invalid, need to log in for {account['username']}")
...
#     username_el = wait.until(lambda d: d.find_element(By.ID, "username"))
#     username_el.clear()
#     password_el = driver.find_element(By.ID, "password")
#     password_el.clear()
#     # Simulate human typing for anti-bot
...
def get_logged_in_username(driver):
    ...
    time.sleep(2)
    try:
        # Find all anchor tags with /in/ in href
        profile_links = driver.find_elements(By.CSS_SELECTOR, "a[href*='/in/']")
        for link in profile_links:
            profile_url = link.get_attribute("href")
            # Skip other /in/ links, find the one that matches your avatar/nav
            if "miniProfile" not in profile_url:
                username = profile_url.split("/in/")[1].strip("/").split("/")[0]
                return username
        return "unknown"
    except Exception as e:
        log.warning(f"Could not fetch LinkedIn username: {e}")
        return "unknown"


def process_batch(batch_urls, account, batch_num):
    driver = init_driver()
    linkedin_username = get_logged_in_username(driver)
    wait = WebDriverWait(driver, 15)
    ...
    for record in batch_urls:
        try:
            ...
            today_date = now_local.date()                    # date object
            today_human = today_date.strftime('%Y-%m-%d')    # string "YYYY-MM-DD"
            ...
            today_date = now_local.date()                    # date object
            today_human = today_date.strftime('%Y-%m-%d')    # string "YYYY-MM-DD"
            lead_zoho_id = record.get('ID')  
            normalized_status = status.strip().title()# Always present if fetched from Zoho

            # --- Handle Pending Status ---
            if status.lower() == "pending":
                # Use your actual field name from Zoho (e.g. 'request_sent_date')
                request_sent_date_str = record.get('request_sent_date')
                try:
                    sent_date = datetime.datetime.strptime(request_sent_date_str, '%Y-%m-%d').date()
                except Exception:
                    sent_date = today_date  # fallback to today

                diff = (today_date - sent_date).days   # <-- FIXED THIS LINE
                ...
                if diff == 0:
                    ...
                    suffix = f'({diff} days ago)'
                human_dt = f'{sent_date.isoformat()} {suffix}'
                ...
                existing = find_records(PARENT_REPORT, {"profile_id": profile_id})
                if existing:
                    zoho_linkedindata_id = existing[0].get("ID")
                    resp = upsert_linkedindata(payload, lead_zoho_id=zoho_linkedindata_id)
                else:
                    resp = upsert_linkedindata(payload)
                log.info(f"[Record {profile_id}] Pending status, connection date: {human_dt} | Response: {resp}")
                random_delay(JITTER)
                continue
            ...
            # --- Handle Already Connected Status ---
                ...
                # --- Correct: Look up in linkedindata using profile_id ---
                existing = find_records(PARENT_REPORT, {"profile_id": profile_id})
                if existing:
                    zoho_linkedindata_id = existing[0].get("ID")
                    resp = upsert_linkedindata(payload, lead_zoho_id=zoho_linkedindata_id)
                    ...
                    resp = upsert_linkedindata(payload)
                log.info(f"[Record {profile_id}] Already Connected, connection date: {raw} | Response: {resp}")
                random_delay(JITTER)
                continue
            ...
            # --- Handle Other Statuses (Request Sent with/without Note) ---
                ...
                profile_payload = {
                    ...
                    'profile_id': profile_id,
                    'invite_message': note,
                    'request_sent_from': linkedin_username,
            ...
            try:
                driver.close()  # Ensure tab is closed on error
                driver.switch_to.window(main_handle)
            except Exception:
                pass
            continue
...
def process_urls_for_account(args):
    ...
    total_batches = (len(assigned_urls) + BATCH_SIZE - 1) // BATCH_SIZE  # Calculate total batches
    for batch_num, batch_urls in enumerate(chunk_list(assigned_urls, BATCH_SIZE), 1):
        process_batch(batch_urls, account, batch_num)
        if batch_num < total_batches:  # Don't sleep after the last batch
            log.info(f"Sleeping for {BATCH_DELAY} seconds before processing next batch...")
...
def main():
    log.info("Starting LinkedIn connection processing...")
    time.sleep(10)
    ...
    log.info("Processing %d LinkedIn URLs with your real Chrome profile", len(records))
    process_urls_for_account((1, {'username': 'example_username'}, records))  # Account info added here
    log.info("✅ All done.")
